...
class CourseRepository {
  Future<CurriculumModel> curriculum(int subscribedCourseId,
      {Map<String, dynamic>? filter}) async {
    final response = await api.get(
        '/subscribed-courses/$subscribedCourseId/curriculum',
  ...
  Future<List<int>> completedLectures(int course) async {
    final response = await api.get(
        '/subscribed-courses/$course/completedlectures',
        queryParameters: {});
  ...
  Future<Response> deleteCompletedLectures(int course, int lecture) async {
    return await api
        .delete('/subscribed-courses/$course/completedlectures/$lecture');
  }

  Future<Response> addCompletedLectures(int course, int lecture) async {
    return await api.post('/subscribed-courses/$course/completedlectures',
        data: {'lecture_id': lecture});
  }
}
